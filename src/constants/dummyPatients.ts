...
export const DUMMY_PATIENTS: Record<string, DummyPatient> = {
  Psychiatry: {
    ...
    history:
      'Pierwsza wizyta kontrolna po 2 tygodniach leczenia. Pacjent zgłasza poprawę nastroju i normalizację snu.',
  },
  'Child Psychiatry': {
    name: 'MN03',
    age: 14,
    ...
    history:
      'Kontrola po 1 miesiącu leczenia. Rodzice zgłaszają spadek apetytu, ale poprawę koncentracji w szkole.',
  },
  Surgery: {
    name: 'AW55',
    age: 55,
    ...
    history:
      'Kwalifikacja do cholecystektomii laparoskopowej. USG potwierdza obecność złogów.',
  },
  'Smart Select': {
    ...
    meds: [],
    history:
      'Ogólna konsultacja medyczna. Pacjent zgłasza się z rutyną kontrolą stanu zdrowia.',
  },
};
...
export const generateSampleNote = (
  ...
  const isConcise =
    templateInstructions.toLowerCase().includes('krótk') ||
  ...
  const focusMeds =
    templateInstructions.toLowerCase().includes('lek') ||
    templateInstructions.toLowerCase().includes('med') ||
    templateInstructions.toLowerCase().includes('farmako');

  const skipFamily =
    templateInstructions.toLowerCase().includes('bez historii rodzinnej') ||
